...
ds004737, ds004869) are wired in a later phase; this module provides the descriptor type,
...
SHELLED_SCHEMES = (SINGLE_SHELL, MULTI_SHELL)
DSI = "DSI"
DMRI_SCHEMES = (*SHELLED_SCHEMES, DSI)
...
def _load_pet(
    ...
    *,
    ...
    ``pairs`` is a list of ``(pet_nii, json)``; typically a single pair
    for a given PET series.

    Parameters
    ----------
    pairs : :obj:`Sequence`
        List of ``(pet_nii, json)`` file paths.
    ...
    for pet_file, json_file in pairs:
        # Load PET NIfTI
        img = load_api(pet_file, SpatialImage)
        affine = img.affine
        data_list.append(np.asarray(img.dataobj, dtype=np.float32))

        # Load tracer metadata and compute midframe times
        with open(json_file, "r") as f:
            tracer_attrs = json.load(f)

        frame_time = tracer_attrs.get(FRAME_TIME_START_KEY, None)
...
def _pet_sidecars(nii: Path) -> tuple[Path, Path]:
    """Return ``(nii, json)`` for a ``*_pet.nii.gz`` file.

    Looks for a JSON sidecar containing ``FrameTimesStart`` information.
    """
    base = str(nii)[: -len(".nii.gz")]
...
def _resolve_ds004869(cache_root=None) -> tuple[Path, list[tuple[Path, Path]]]:
    ds = _ensure_clone("ds004869", cache_root)
    return ds, [_pet_sidecars(_first(ds, "sub-01/ses-baseline/pet/*_pet.nii.gz"))]
...
def load_ds004869(cache_root: str | Path | None = None) -> PET:
    """PET ([11C]MC1): ds004869."""
    ds, doubles = _resolve_ds004869(cache_root)
    return _load_pet(doubles, ds)
...
DATASETS: list[DatasetSpec] = [
    ...
    DatasetSpec(
        name="ds004869",
        title="[11C]MC1 PET (ds004869)",
        scheme=PET_SCHEME,
        loader=load_ds004869,
        notes="PET.",
        url="https://openneuro.org/datasets/ds004869",
    ),
]
